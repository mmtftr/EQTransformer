...
def preprocessor(preproc_dir, mseed_dir, stations_json, overlap=0.3, n_processor=None):
    ...
    def process(station_dir):
    # for station in station_list:
        if platform.system() == 'Windows':
            output_name = station_dir.split("\\")[-1];
        else:
            output_name = station_dir.split("/")[-1]
        ...
        if platform.system() == 'Windows':
            file_list = [join(station_dir, ev) for ev in listdir(station_dir) if ev.split("\\")[-1] != ".DS_Store"];
        else:
            file_list = [join(station_dir, ev) for ev in listdir(station_dir) if ev.split("/")[-1] != ".DS_Store"];

        file_time_ranges = [ev.split('__')[1]+'__'+ev.split('__')[2] for ev in file_list ];
        sorted_time_ranges = list(set(file_time_ranges))
        ...
        if platform.system() == 'Windows':
            print('============ Station {} has {} chunks of data.'.format(station_dir.split("\\")[-1], len(sorted_time_ranges)), flush=True)
        else:
            print('============ Station {} has {} chunks of data.'.format(output_name, len(sorted_time_ranges)), flush=True)

        count_chuncks=0; fln=0; c1=0; c2=0; c3=0; fl_counts=1; slide_estimates=[];

        for ct, month in enumerate(sorted_time_ranges):
            ...
            if len(matching) == 3:
                ...
                if platform.system() == 'Windows':
                    print('  * '+station_dir.split("\\")[-1]+' ('+str(count_chuncks)+') .. '+month.split('T')[0]+' --> '+month.split('__')[1].split('T')[0]+' .. 3 components .. sampling rate: '+str(org_samplingRate))
                else:
                    print('  * '+output_name+' ('+str(count_chuncks)+') .. '+month.split('T')[0]+' --> '+month.split('__')[1].split('T')[0]+' .. 3 components .. sampling rate: '+str(org_samplingRate))
                ...
                while next_slice <= end_time:
                    ...
                    if platform.system() == 'Windows':
                        dsF.attrs["receiver_code"] = station_dir.split("\\")[-1]
                        dsF.attrs["network_code"] = stations_[station_dir.split("\\")[-1]]['network']
                        dsF.attrs["receiver_latitude"] = stations_[station_dir.split("\\")[-1]]['coords'][0]
                        dsF.attrs["receiver_longitude"] = stations_[station_dir.split("\\")[-1]]['coords'][1]
                        dsF.attrs["receiver_elevation_m"] = stations_[station_dir.split("\\")[-1]]['coords'][2]
                    else:
                        dsF.attrs["receiver_code"] = output_name
                        dsF.attrs["network_code"] = stations_[output_name]['network']
                        dsF.attrs["receiver_latitude"] = stations_[output_name]['coords'][0]
                        dsF.attrs["receiver_longitude"] = stations_[output_name]['coords'][1]
                        dsF.attrs["receiver_elevation_m"] = stations_[output_name]['coords'][2]
            ...
            if len(matching) == 1:
                 ...
                 if platform.system() == 'Windows':
                     print('  * '+station_dir.split("\\")[-1]+' ('+str(count_chuncks)+') .. '+month.split('T')[0]+' --> '+month.split('__')[1].split('T')[0]+' .. 1 components .. sampling rate: '+str(org_samplingRate))
                 else:
                     print('  * '+output_name+' ('+str(count_chuncks)+') .. '+month.split('T')[0]+' --> '+month.split('__')[1].split('T')[0]+' .. 1 components .. sampling rate: '+str(org_samplingRate))
            ...
            if len(matching) == 2:
                ...
                if platform.system() == 'Windows':
                    print('  * '+station_dir.split("\\")[-1]+' ('+str(count_chuncks)+') .. '+month.split('T')[0]+' --> '+month.split('__')[1].split('T')[0]+' .. 2 components .. sampling rate: '+str(org_samplingRate))
                else:
                    print('  * '+output_name+' ('+str(count_chuncks)+') .. '+month.split('T')[0]+' --> '+month.split('__')[1].split('T')[0]+' .. 2 components .. sampling rate: '+str(org_samplingRate))
        ...
        assert count_chuncks == len(sorted_time_ranges)
        assert sum(slide_estimates)-(fln/100) <= len(dd) <= sum(slide_estimates)+10
        data_track[output_name]=[time_slots, comp_types]
        print(f" Station {output_name} had {len(sorted_time_ranges)} chuncks of data")
        ...
        try:
            ...
            repfile.write(f' Station {output_name} had {len(sorted_time_ranges)} chuncks of data, {len(dd)} slices were written, {int(sum(slide_estimates))} were expected. Number of 1-components: {c1}, Number of 2-components: {c2}, number of 3-components: {c3}, original samplieng rate: {org_samplingRate}\n')
        except Exception:
            pass
